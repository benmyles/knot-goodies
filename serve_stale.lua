-- MIT License
--
-- serve_stale.lua is an alternative version of the knot resolver serve_stale module that always serves
-- stale data when the TTL is expired, similar to "optimistic caching" in AdGuard Home.
--
-- It sets a short 10s TTL on the returned record and kicks off an async resolve to fetch fresh data
-- for future use.
--
-- Install via:
...
log_debug(ffi.C.LOG_GRP_SRVSTALE, '   => loading always_serve_stale module')
...
M.callback = ffi.cast("kr_stale_cb",
    function(ttl, name, type, qry)
        ...
        if ttl + 3600 * 24 * 3 > 0 then -- at most 3 days stale
            log_debug(ffi.C.LOG_GRP_SRVSTALE, '   => served stale data for ' .. n .. ' with TTL: ' .. tostring(ttl))
